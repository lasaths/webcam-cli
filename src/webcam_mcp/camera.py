...
class WebcamCapture:
    ...
    def _set_resolution(self, cap: cv2.VideoCapture, width: int, height: int) -> tuple[int, int]:
        ...
        if (actual_w, actual_h) != (width, height):
            logger.warning(
                ...
                f"but camera provided {actual_w}x{actual_h}"
            )
    ...
    def capture_photo(self, width: int = 1920, height: int = 1080, 
    ...
    def capture_video_frames(self, duration_seconds: float, width: int = 640, 
                             height: int = 480, max_frames: int = 50, 
                             quality: int = 75) -> list[bytes]:
